...
describe('getOverallScreenshotOutcome()', function () {
  it('should return true for a single passing result', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome({ screenshotComparePassing: true })
    assert.strictEqual(passing, true)
  })
  it('should return false for a single failing result', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome({ screenshotComparePassing: false })
    assert.strictEqual(passing, false)
  })
  it('should return null for a single unknown status result', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome({ screenshotComparePassing: null })
    assert.strictEqual(passing, null)
  })
  it('should return null for a single result without a passing field', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome({})
    assert.strictEqual(passing, null)
  })
  it('should return true for multiple results all passing', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome([{ screenshotComparePassing: true }, { screenshotComparePassing: true }])
    assert.strictEqual(passing, true)
  })
  it('should return false for multiple results all failing', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome([{ screenshotComparePassing: false }, { screenshotComparePassing: false }])
    assert.strictEqual(passing, false)
  })
  it('should return false for multiple results some failing', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome([{ screenshotComparePassing: false }, { screenshotComparePassing: true }])
    assert.strictEqual(passing, false)
  })
  it('should return null for multiple results some unknown', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome([{ screenshotComparePassing: true }, { screenshotComparePassing: null }])
    assert.strictEqual(passing, null)
  })
  it('should return null for empty results array', async () => {
    const passing = GhostInspector.getOverallScreenshotOutcome([])
